...
fn main() {
    ...
    let gamma_binary = bits.iter().fold("".to_string(), |acc, &(zeroes, ones)| {
        acc + if ones > zeroes { "1" } else { "0" }
    });

    let gamma = i32::from_str_radix(&gamma_binary, 2).unwrap();

    let epsilon_binary = bits.iter().fold("".to_string(), |acc, &(zeroes, ones)| {
        acc + if ones < zeroes { "1" } else { "0" }
    });
    ...
    for i in 0..input[0].len() {
        let bits: Vec<char> = o2_candidates
            .iter()
        ...
        if o2_candidates.len() != 1 {
            o2_candidates = o2_candidates
                .into_iter()
                .filter(|n| n.chars().nth(i).unwrap() == most_common)
                .collect::<Vec<String>>();
        }

        let bits: Vec<char> = co2_candidates
            .iter()
            .map(|n| n.chars().nth(i).unwrap())
            .collect();
        let zeroes = bits.iter().filter(|b| **b == '0').count();
        ...
        if co2_candidates.len() != 1 {
            co2_candidates = co2_candidates
                .into_iter()
                .filter(|n| n.chars().nth(i).unwrap() == least_common)
                .collect::<Vec<String>>();
        }
    }
